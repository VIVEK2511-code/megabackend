...
import { SubTask } from "../models/subtasks.models.js";
  ...
  const uploadTaskAttachments = async (files) => {
    ...
    for (const file of files) {
       // file.path jo aap uploadTaskAttachments me use kar rahe ho, wo Multer ke storage engine (mostly DiskStorage) se aata hai.
    //    calls uploadOnCloudinary(file.path, "tasks"):
// file.path → The temporary path on your server where the file is stored after being uploaded by Multer.
// "tasks" → The Cloudinary folder where the file should be uploaded.
// await means → wait until Cloudinary finishes uploading this file before moving to the next.
        const uploaded = await uploadOnCloudinary(file.path, "tasks");
//         uploaded ke andar Cloudinary ka full response object aata hai.

// Us response me kaafi fields hoti hain (public_id, url, width, height, format, created_at, etc.).
        if (uploaded) {
 ...
 export const createTask = asyncHandler(async (req, res) => {
  ...
  export const getTasksOfProject = asyncHandler(async (req, res) => {
    const { projectId } = req.params;
...
export const updateTaskStatusOrPriority = asyncHandler(async (req, res) => {
    const { taskId } = req.params;
    if (!taskId) throw new ApiError(400, "Task id is required");
  ...
  export const updateTaskAttachments = asyncHandler(async (req, res) => {
    if (!req.files || !req.files.attachments) {
        throw new ApiError(400, "No attachments provided");
    }
  ...
  export const getUserAssignedTasks = asyncHandler(async (req, res) => {
